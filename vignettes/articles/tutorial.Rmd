...
All operations supported by this package work without an API key, but you might get rate limited without an API key.
The API key can be specified via the option `mlr3oml.apy_key` or the environment variable `OPENMLAPIKEY` (where the latter has precedence over the former).
