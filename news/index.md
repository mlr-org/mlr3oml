...
- Add `uploader` argument to
  [`list_oml_data()`](https://mlr3oml.mlr-org.com/reference/list_oml.md).
...
- Added
  [`list_oml_collections()`](https://mlr3oml.mlr-org.com/reference/list_oml.md)
  to list collections (run or task collections) on OpenML.
...
- Fix: Parquet datasets now work where columns simultaneously have to be
...
- Removed `benchmark_grid_oml()`, which was already deprecated in
  release 0.7.2.
- Removed the fields `runs`, `flows`, `data`, `tasks` from the
  `OMLCollection` class. Consequently, the `cache` option can no longer
  be set for `OMLCollection` objects, see the class documentation for
  more information.
...
- Caching can no longer be specified at the instance level but only
  globally through the option `mlr3oml.cache`
- Added `$download()` method for all OML objects to fully download an
  object for offline usage.
...
- Fix: target is added to features when converting a `OMLData` object to
  a task with an explicit target variable that is **not** the default
  target.
- Deprecated `benchmark_grid_oml()` in favour of
...
- Fix: Incremented cache version for data description and fixed bug, as
  [`make.names()`](https://rdrr.io/r/base/make.names.html) was not
...
- Fixed argument names of S3 method for `as_data_backend` to comply with
  new CRAN checks
...
- feature: Add argument `task_type` to function
  [`list_oml_tasks()`](https://mlr3oml.mlr-org.com/reference/list_oml.md).
...
- Renamed the sugar functions from:
  - [`oml_data()`](https://mlr3oml.mlr-org.com/reference/oml_data.md) is
    now [`odt()`](https://mlr3oml.mlr-org.com/reference/odt.md)
  - [`oml_task()`](https://mlr3oml.mlr-org.com/reference/oml_task.md)is
    now [`otsk()`](https://mlr3oml.mlr-org.com/reference/otsk.md)
  - [`oml_flow()`](https://mlr3oml.mlr-org.com/reference/oml_flow.md) is
    now [`oflw()`](https://mlr3oml.mlr-org.com/reference/oflw.md)
  - [`oml_run()`](https://mlr3oml.mlr-org.com/reference/oml_run.md) is
    now `orn`
  - [`oml_collection()`](https://mlr3oml.mlr-org.com/reference/oml_collection.md)
    is now [`ocl()`](https://mlr3oml.mlr-org.com/reference/ocl.md)
...
- Conversion from OpenML to mlr3 objects is now only possible with the
  usual s3-converters `as_<object>`. This improves consistency by
  ensuring that the subcomponents of OpenML objects are always OpenML
  objects and not suddenly mlr3 objects.
...
- Added support for parquet files that were recently introduced on
  OpenML. The global option `mlr3oml.parquet` can be used to enable or
  disable this. By default it is `FALSE`. This is implemented via the
  duckdb backend from `mlr3db`.
- Support to use the OpenML test server. This can be globally enabled
  using the option `mlr3oml.test_server` or individually for objects.
  Options to globally define an API-key for the test server are through
  the environment variable `TESTOPENMLAPIKEY` or the option
...
- Implement an arff writer and remove the arff dependency, therefore
  also removing the option `"farff"` as the `mlr3oml.arff_parser`
- Increment the cache version number due to changes in the cache
  structure: This will flush the previous cache folder.
- Simplified the code structure by adding `OMLObject` class from which
  all other OpenML objects like `OMLData`, `OMLTask` inherit.
...
- More functions to list objects from OpenML:
  - [`list_oml_evaluations()`](https://mlr3oml.mlr-org.com/reference/list_oml.md)
  - [`list_oml_flows()`](https://mlr3oml.mlr-org.com/reference/list_oml.md)
  - [`list_oml_measures()`](https://mlr3oml.mlr-org.com/reference/list_oml.md)
  - [`list_oml_runs()`](https://mlr3oml.mlr-org.com/reference/list_oml.md)
  - [`list_oml_setups()`](https://mlr3oml.mlr-org.com/reference/list_oml.md)
...
- If not set via option `mlr3oml.api_key`, the API key is retrieved from
  the environment variable `OPENMLAPIKEY`.
- Implemented a retry mechanism as a workaround for temporary connection
  errors.
...
- The connectors used in `mlr_tasks` and `mlr_resamplings` now signal
  errors of class `missingDefaultError` if some defaults are not set.
- Target columns are now automatically converted to the require storage
  mode during task creation.
...
- Support filtering data sets and tasks via data id or task id
  ([\#5](https://github.com/mlr-org/mlr3oml/issues/5)).
- Added fallback to RWeka for sparse ARFF files
  ([\#6](https://github.com/mlr-org/mlr3oml/issues/6)).
