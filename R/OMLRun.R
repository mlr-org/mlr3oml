...
OMLRun = R6Class("OMLRun",
  public = list(
    ...
    print = function() {
      ...
      catf(" * Data: %s (%s)", self$task$data_name, self$data_id)
      catf(" * Task ID: %s", self$task_id)
      catf(" * Data Split: %s", self$task$data_split$type)
      catf(" * Flow: %s (%s)", as_short_string(self$flow$name), self$flow_id)
    }
  ),
  active = list(
    #' @field desc (`list()`)\cr
